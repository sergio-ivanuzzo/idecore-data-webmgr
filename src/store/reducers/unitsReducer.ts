...
import { UnitsActionType } from '~store/actions';
...
export const unitsReducer = (state = initialState, action: AnyAction): IUnitReducerState => {
	switch (action.type) {
		case UnitsActionType.UNITS_FETCH_LIST_COMPLETE: {
			return {
				...state,
				unitItems: action.payload,
			};
		}
		default: {
			return state;
